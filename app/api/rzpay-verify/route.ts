		...
		if (order?.notes?.type === "pass") {
            ...
            <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #2c2c2c; padding: 24px;">
				...
				<p style="font-size: 16px; margin-bottom: 20px;">
					...
					<strong style="color: #5a3e2b;">₹${Number(order.amount)/100}</strong> has been successfully processed for 
			...
			const registration = new Registration({
				...
				amount: Number(order.amount) / 100,
				type: order.notes.type,
				classId: order.notes.eventId,
				noOfParticipants: order.notes.teamSize,
				participantsData: (Array.isArray(order.notes.teamMembers) ? order.notes.teamMembers : []).map((memberName: string) => ({
					name: memberName,
					arrived: false,
				})),
			});
			...
			const emailHtml = `
				<div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #2c2c2c; padding: 24px;">
					...
					<p style="font-size: 16px; margin-bottom: 20px;">
						We are pleased to inform you that your payment of 
						<strong style="color: #5a3e2b;">₹${Number(order.amount)/100}</strong> has been successfully processed for 
						<strong style="color: #5a3e2b;">${order.notes.eventId}</strong>.
					</p>
					...
					<ul style="list-style: none; padding: 0; font-size: 15px; margin-bottom: 20px;">
						${(Array.isArray(order.notes.teamMembers) ? order.notes.teamMembers : []).map(
								(memberName: string) =>
									`<li style="margin-bottom: 8px;">${memberName}</li>`
							)
							.join("")}
					</ul>
					<p style="font-size: 16px; margin-bottom: 24px;">Thank you for your registration. We’re excited to have you on board.</p>
			...
			try {
				await sendEmail(
					`${order.notes.email}`,
					`Payment Confirmation: ${razorpay_payment_id}`,
					`Your payment of ${order.amount} has been successfully processed for ${order.notes.eventId}.`,
					emailHtml
				);
				console.log("Email sent successfully");
			} catch (error) {
