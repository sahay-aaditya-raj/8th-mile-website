...
export default function Navbar() {
  ...
  // Separate logo for mobile menu centering
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 flex items-center justify-between relative h-16 z-10">
        {/* RVCE Logo */}
        ...
        {/* Mobile Menu Overlay */}
        {isMenuOpen && (
          <div className="fixed inset-0 top-16 bg-black/80 backdrop-blur-sm z-10 flex justify-center pt-8 px-4 md:hidden">
            <div className="w-full max-w-md bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/10 shadow-2xl shadow-black/20 overflow-y-auto max-h-[80vh]">
                ...
                {otherLinks.map((link) => (
                  <li key={link.name}>
                    <Button
                      variant="ghost"
                      className={cn(
                        "w-full justify-start text-white text-lg py-4 px-6 transition-colors",
                        "flex hover:bg-white/20",
                        pathname === link.href && "bg-white/20 font-medium"
                      )}
